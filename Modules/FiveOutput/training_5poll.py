...
from Modules.FiveOutput.transforms_5poll import ChangeBandOrder, ToTensor, DatasetStatistics, Normalize, Randomize
from model_3poll import get_model
from Modules.FiveOutput.utils_5poll import load_data, none_or_true, dotdict, set_seed, step
from Modules.FiveOutput.train_utils_5poll import eval_metrics, split_samples,  test, test_plotter

parser = argparse.ArgumentParser(description='train_multimodel')
network = "resnet18" #alternatively: "resnet50","resnet18"

# block for complete training
parser.add_argument('--samples_file', default="data/multimodal/5_poll_data_train.csv", type=str)
parser.add_argument('--datadir', default="D:\VS_code\AQNet\Datadir\eea", type=str)
    ...
    with mlflow.start_run(experiment_id=experiment_id):
        ...
        for epoch in range(config.epochs):
            ...
            for idx, sample in enumerate(dataloader_train):
                ...
                #add
                Onehot7 = sample["rural-nearcity"]
                Onehot8 = sample["rural-regional"]

                tabular = [sample["Altitude"],Onehot1,Onehot2,Onehot3,Onehot4,Onehot5,Onehot6,Onehot7, Onehot8, 
                ...
                y_4 = sample["co"].float()
                y_5 = sample["so2"].float()

                y_samples = [y_1, y_2, y_3, y_4, y_5]
            ...
            valid_val = (val_y_hat["no2"] > 0)&(val_y_hat["no2"] < 100)&\
                        ...
                        (val_y_hat["co"] > 0)&(val_y_hat["co"] < 100)&\
                        (val_y_hat["so2"] > 0)&(val_y_hat["so2"] < 100)
            ...
            eval_val_co = eval_metrics(val_y["co"], val_y_hat["co"])
            eval_val_so2 = eval_metrics(val_y["so2"], val_y_hat["so2"])
            eval_val = {"no2":eval_val_no2,"o3":eval_val_o3,"pm10":eval_val_pm10,"co":eval_val_co,"so2":eval_val_so2}
            ...
            print(f"Epoch: {epoch} 'co; r2,mae,mse' {eval_val['co']}")
            print(f"Epoch: {epoch} 'so2; r2,mae,mse' {eval_val['so2']}")

            performances_val.append([run, epoch] + eval_val["no2"] + eval_val["o3"] + eval_val["pm10"] + eval_val["co"] + eval_val["so2"] )
            mlflow.log_metrics({"val_r2_epoch_no2" : eval_val["no2"][0],"val_mae_epoch_no2" : eval_val["no2"][1],"val_mse_epoch_no2" : eval_val["no2"][2],
                                ...
                                "val_r2_epoch_co": eval_val["co"][0],"val_mae_epoch_co": eval_val["co"][1],"val_mse_epoch_co": eval_val["co"][2],
                                "val_r2_epoch_so2": eval_val["so2"][0],"val_mae_epoch_so2": eval_val["so2"][1],"val_mse_epoch_so2": eval_val["so2"][2]}, step=epoch)
        ...
        valid_test = (test_y_hat["no2"] > 0)&(test_y_hat["o3"] > 0)&(test_y_hat["pm10"] > 0)&(test_y_hat["co"] > 0)&(test_y_hat["so2"] > 0)
        valid_train = (train_y_hat["no2"] > 0)&(train_y_hat["o3"] > 0)&(train_y_hat["pm10"] > 0)&(train_y_hat["co"] > 0)&(train_y_hat["so2"] > 0)
        ...
        eval_test_co = eval_metrics(test_y["co"], test_y_hat["co"])
        eval_test_so2 = eval_metrics(test_y["so2"], test_y_hat["so2"])
        eval_test = {"no2": eval_test_no2,"o3": eval_test_o3,"pm10": eval_test_pm10,"co": eval_test_co,"so2": eval_test_so2}
        ...
        eval_train_co = eval_metrics(train_y["co"], train_y_hat["co"])
        eval_train_so2 = eval_metrics(train_y["so2"], train_y_hat["so2"])
        eval_train = {"no2": eval_train_no2,"o3": eval_train_o3,"pm10": eval_train_pm10,"co": eval_train_co,"so2": eval_train_so2}
        ...
        performances_test.append([eval_test_no2[0],eval_test_no2[1],eval_test_no2[2],
                                  ...
                                  eval_test_co[0],eval_test_co[1],eval_test_co[2],
                                  eval_test_so2[0],eval_test_so2[1],eval_test_so2[2]])
        performances_train.append([eval_train_no2[0],eval_train_no2[1],eval_train_no2[2],
                                  ...
                                  eval_train_co[0],eval_train_co[1],eval_train_co[2],
                                  eval_train_so2[0],eval_train_so2[1],eval_train_so2[2]])
