...
def test(model, dataloader, device, datastats):
    ...
    with torch.no_grad():
        for idx, sample in enumerate(dataloader):
            ...
            # Onehot9 = sample["tavg"]
            # Onehot10 = sample["prcp"]
            # Onehot11 = sample["wspd"]
            # Onehot12 = sample["pres"]            
            # Onehot13 = sample["tsun"]

            tabular = [sample["Altitude"], Onehot1, Onehot2, Onehot3, Onehot4, Onehot5, Onehot6, Onehot7, Onehot8, sample["tavg"], sample["prcp"], sample["wspd"], sample["pres"], sample["tsun"]]
            tabular = torch.stack(tabular,dim=1).float().to(device)
            model_input = {"img": img, "s5p": s5p, "tabular": tabular}

            y_hat_1,y_hat_2,y_hat_3,y_hat_4,y_hat_5 = model(model_input)
            y_hat_no2 = y_hat_1.squeeze()
            y_hat_o3 = y_hat_2.squeeze()
            y_hat_pm10 = y_hat_3.squeeze()
            y_hat_co = y_hat_4.squeeze()
            y_hat_so2 = y_hat_5.squeeze()

            y_no2 = sample["no2"].float().to(device).squeeze()
            y_o3 = sample["o3"].float().to(device).squeeze()
            y_pm10 = sample["pm10"].float().to(device).squeeze()
            y_co = sample["co"].float().to(device).squeeze()
            y_so2 = sample["so2"].float().to(device).squeeze()
            ...
            measurements_co.append(y_co.cpu().numpy().item())
            measurements_so2.append(y_so2.cpu().numpy().item())
            ...
            predictions_co.append(y_hat_co.cpu().numpy().item())
            predictions_so2.append(y_hat_so2.cpu().numpy().item())
    ...
    measurements_co = Normalize.undo_co_standardization(datastats, np.array(measurements_co))
    measurements_so2 = Normalize.undo_so2_standardization(datastats, np.array(measurements_so2))
    ...
    predictions_co = Normalize.undo_co_standardization(datastats, np.array(predictions_co))
    predictions_so2 = Normalize.undo_so2_standardization(datastats, np.array(predictions_so2))

    measurements = {"no2":measurements_no2,
                    "o3":measurements_o3,
                    "pm10":measurements_pm10,
                    "co":measurements_co,
                    "so2":measurements_so2}
