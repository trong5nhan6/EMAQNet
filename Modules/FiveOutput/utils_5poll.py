...
from Modules.FiveOutput.train_utils_5poll import eval_metrics
...
def step(x, y_samples, model, loss, optimizer):
    ...
    y_no2, y_o3, y_pm10, y_co, y_so2 = y_samples
    y_hat_1,y_hat_2,y_hat_3,y_hat_4,y_hat_5 = model(x)

    y_train = torch.stack([y_no2, y_o3, y_pm10, y_co, y_so2])
    y_epoch = torch.stack([y_hat_1,y_hat_2,y_hat_3,y_hat_4,y_hat_5])
...
def load_data(datadir, samples_file):
    ...
    try:
        ...
        for station in tqdm(samples_df.AirQualityStation.unique()):
            ...
            s5p_data = xr.open_dataset(os.path.join(datadir, "sentinel-5p", s5p_path), engine="netcdf4").rio.write_crs(4326)

            for idx in station_obs.index.values:
                sample = samples_df.loc[idx].to_dict() # select by index value, not position
                sample["idx"] = idx
                sample["s5p"] = s5p_data.tropospheric_NO2_column_number_density.values[sample['Month']].squeeze()
