...
def get_model(device, network, tabular_switch, S5p_switch, checkpoint=None):
    ...
    # S2: ResNet18
    # backbone_S2 = resnet18(pretrained=checkpoint)
    # backbone_S2.conv1 = nn.Conv2d(12, 64, kernel_size=7, stride=2, padding=3, bias=False)
    ...
    # S5P: MobileNetV2
    backbone_S5P = mobilenet_v2(pretrained=checkpoint)
    backbone_S5P.features[0][0] = nn.Conv2d(1, 32, kernel_size=3, stride=2, padding=1)
    backbone_S5P.classifier = nn.Sequential(
        nn.Linear(backbone_S5P.last_channel, S5p_num_features),
        nn.ReLU(),
        nn.Dropout(0.25)
    ...
    if tabular_switch == True:  # use s2, s5p and tabular
        regression_model = RegressionHead_1(
            backbone_S2, backbone_S5P, head_1, backbone_tabular, mixer_1)
    else:                       # use s2, s5p
